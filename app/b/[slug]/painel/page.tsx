...
export default async function BarracaPainelResolverPage({
  params,
}: {
  params: { slug: string };
}) {
  ...
  // Pega um conjunto e compara normalizado (robusto)
  ...
  if (exact?.id) {
    redirect(`/painel?b=${exact.id}`);
  }

  // Fallback: busca permissiva (se der 1, redireciona)
  ...
  if (candidatos && candidatos.length === 1 && candidatos[0]?.id) {
    redirect(`/painel?b=${candidatos[0].id}`);
  }

  // Diagnóstico (evita 404 cego)
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      <div className="mx-auto max-w-2xl px-4 py-10">
        <h1 className="text-xl font-semibold">Painel: barraca não encontrada</h1>
