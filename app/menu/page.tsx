...
export default function MenuPage() {
  ...
  // ✅ Produtos ativos por categoria (para esconder categorias vazias)
  ...
  // ✅ Só categorias que têm pelo menos 1 produto ativo
  const categoriasVisiveis = useMemo(() => {
    return categorias.filter((c) => (prodsByCat.get(c.id)?.length ?? 0) > 0);
  }, [categorias, prodsByCat]);
    ...
    const [{ data: cats, error: cErr }, { data: prods, error: pErr }] =
      await Promise.all([
        ...
        supabase
          ...
          .eq("barraca_id", barracaId)
          .eq("ativo", true) // ✅ garante que produto inativo não aparece no menu
          .order("nome", { ascending: true }),
      ]);
  ...
  // ✅ Ajusta activeCat quando categorias/produtos carregam:
  // - se activeCat atual ficou invisível, troca para a primeira categoria visível
  useEffect(() => {
    if (loading) return;
    ...
    const stillValid = activeCat && categoriasVisiveis.some((c) => c.id === activeCat);
    if (!stillValid) setActiveCat(categoriasVisiveis[0].id);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, categoriasVisiveis.map((c) => c.id).join("|")]);
  ...
  const produtosDaCategoria = useMemo(() => {
    if (!activeCat) return produtos;
    const list = produtos.filter((p) => p.categoria_id === activeCat);
    return list;
  }, [produtos, activeCat]);

  // ✅ Se um produto saiu do menu (ficou inativo), limpa do carrinho automaticamente
  useEffect(() => {
    if (loading) return;

    const validIds = new Set(produtos.map((p) => p.id));
    setCart((prev) => {
      let changed = false;
    ...
    <main className="min-h-screen bg-slate-50">
      <div className="max-w-xl ml-4 md:ml-8 mr-auto p-4 md:p-6 pb-28">
        <header className="mb-5">
          <h1 className="text-2xl md:text-3xl font-semibold tracking-tight">Cardápio</h1>
          <p className="text-slate-500 mt-1 text-sm">
            Escolha seus itens e finalize em segundos
          </p>
        </header>

        {!loading && categoriasVisiveis.length > 0 && (
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      <div className="text-slate-500 mt-1">{brl(Number(p.preco))}</div>
                    </div>
